...
import type { ExternalTypesCtx, GeneratorPlugin } from '../config';
import { collectTypeRefNames, findImportOf, isPrismaModule } from '../ast-helpers';
 ...
 */
function extractStringLiteralsFromTypeLiteral(typeNode: import('ts-morph').Node | undefined): string[] {
    if (!typeNode || !Node.isTypeLiteral(typeNode)) return [];
    const values: string[] = [];
    for (const member of typeNode.getMembers()) {
        if (!Node.isPropertySignature(member)) continue;
        const mType = member.getTypeNode();
        if (mType?.getKind() === SyntaxKind.LiteralType) {
            const literal = mType.getFirstChildByKind(SyntaxKind.StringLiteral);
            if (literal) values.push(literal.getLiteralText());
        }
    }
    return values;
}

function findPrismaEnumValuesFromDts(
    typeName: string,
    fromFile: import('ts-morph').SourceFile,
): string[] {
    for (const decl of fromFile.getImportDeclarations()) {
        if (!isPrismaModule(decl.getModuleSpecifierValue())) continue;
        for (const spec of decl.getNamedImports()) {
            const orig = spec.getName();
            const alias = spec.getAliasNode()?.getText();
            if (orig !== typeName && alias !== typeName) continue;
 ...
 */
function parsePrismaEnumsFromSchema(schemaText: string): Map<string, string[]> {
    ...
    while ((match = enumRegex.exec(schemaText)) !== null) {
        ...
        if (values.length > 0) out.set(enumName, values);
    }
    return out;
}
 ...
 */
export const prismaPlugin: GeneratorPlugin = {
    name: 'prisma',

    async resolveExternalTypes(ctx: ExternalTypesCtx): Promise<void> {
        // Шаг 1: собрать все имена, импортированные из @prisma/* в моделях
        const needed = new Map<string, import('ts-morph').SourceFile>();
        for (const decl of ctx.sorted) {
            const file = decl.getSourceFile();
            for (const prop of decl.getProperties()) {
                for (const ref of collectTypeRefNames(prop.getTypeNode())) {
                    if (ctx.externalTypeAliases.has(ref)) continue;
                    const imp = findImportOf(file, ref);
                    if (imp && isPrismaModule(imp.module)) {
                        needed.set(ref, file);
                    }
                }
            }
        ...
        const schemaPath = path.join(ctx.cfg.repoRoot, 'apps/backend/prisma/schema.prisma');
        const schemaText = await fs.readFile(schemaPath, 'utf-8').catch(() => '');
